    ...
    { 
        ...
        public void DisplayCashFreePayment(CashFreeObject cashFreeObject, string price)
        {
            try
            {
                ...
                CashFreeWindow = new Dialog(ActivityContext, AppTools.IsTabDark() ? Resource.Style.MyDialogThemeDark : Resource.Style.MyDialogTheme);
        ...
        private async Task CashFree(CashFreeGetStatusObject statusObject, string request)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    var (apiStatus, respond) = await RequestsAsync.Payment.CashFreeAsync(request, keyValues);
                    switch (apiStatus)
                    {
                        case 200:
            ...
            public override async void OnPageStarted(WebView view, string url, Bitmap favicon)
            {
                ...
                { 
                    if (url.Contains("requests.php?f=cashfree"))
                    {
                        ...
                        var (apiStatus, respond) = await RequestsAsync.Payment.CashFreeGetStatusAsync(MActivity.CashFreeObject.AppId, ListUtils.MySettingsList?.CashfreeSecretKey ?? "", MActivity.CashFreeObject.OrderId, ListUtils.MySettingsList?.CashfreeMode);
                        switch (apiStatus)
                        {
                            case 200:
                            {
                                switch (respond)
                                {
                                    case CashFreeGetStatusObject result:
                                        await MActivity.CashFree(result, "wallet_paid"); 
                                        break;
                                }
